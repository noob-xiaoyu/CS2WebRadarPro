...
{
	// 受保护进程（VAC）下 toolhelp 快照会被拒绝，改用 psapi 枚举模块
	HMODULE modules[1024] = {};
	DWORD needed = 0;
	if (!EnumProcessModulesEx(this->m_handle, modules, sizeof(modules), &needed, LIST_MODULES_ALL))
		return {};

	const auto count = static_cast<size_t>(needed / sizeof(HMODULE));
	...
	for (size_t i = 0; i < count; i++)
	{
		...
		if (!GetModuleBaseNameW(this->m_handle, modules[i], name, MAX_PATH))
			continue;

		// 转换为小写 UTF-8 比较
		std::string narrow;
		for (const wchar_t* c = name; *c; c++)
			narrow += static_cast<char>(*c & 0x7F);

		if (!equals_ignore_case(narrow, module_name))
			continue;

		MODULEINFO info = {};
		if (!GetModuleInformation(this->m_handle, modules[i], &info, sizeof(info)))
			return {};

		return std::make_pair(reinterpret_cast<uintptr_t>(info.lpBaseOfDll), static_cast<uintptr_t>(info.SizeOfImage));
	}
